...
2. На форме регистрации заполните **сразу четыре поля** (это один экран):
 ...
 - **Имя**.
 - **Пароль** — требования: минимум 8 символов, одна заглавная и одна строчная буква, одна цифра, один спецсимвол. Гарантированно работают `* $ % @ ?` (форма ЛК принимает только их). Сервер сам по себе принимает любой не-буквенно-цифровой символ, но во фронте есть более строгая проверка — поэтому если используете другой символ, форма может не пройти.
...
- AI-юрист Грут — 🚧 *в разработке.* После запуска будет доступен только на Премиуме и выше. На Триале и Профи не появится.

## Шаг 2. Настройка профиля

В кабинете откройте **Профиль** — иконка **ПРОФИЛЬ** в нижнем меню кабинета (крайняя справа). Затем нажмите **«Редактировать»** в правом верхнем углу карточки «Ваши данные».

Заполните:
- **Имя, фамилия, отчество** — как должны видеть клиенты.
- **Email** — для уведомлений о лидах, счетах, новостях продукта.
- **Город** — по нему определяется регион (Столица / Регионы), от этого зависят цены на услуги. **Страна** — заполняется отдельным полем.
- **Пол** — отображается в публичной карточке.
- **Подменный телефон** (Fake-номер, опционально) — публичный номер для объявлений. Если заполнен, клиенты видят его, а не ваш основной. Полезно, если не хотите палить личный телефон.
...
2. Нажмите **«Привязать»** — откроется ссылка на Telegram-бота RSpace (точное имя бота актуально только у поддержки; всегда переходите по кнопке из кабинета).
...
1. В нижнем меню кабинета выберите **«Объекты»**.
2. Нажмите **«Создать объект»** — большая зелёная кнопка под списком (или над пустой страницей, если объектов ещё нет).
...
Когда напротив всех разделов объекта зелёные ✓, в нижней части страницы редактирования появляется крупная зелёная кнопка **«Опубликовать объявление →»**.

1. Нажмите её — попадёте на экран выбора площадок.
2. Выберите: **Авито** и **ЦИАН**.
3. Подтвердите.

Если открыли список объектов, а не редактирование, к этому же экрану можно попасть через меню «⋮» в карточке объекта → **«Создать объявление»**.
