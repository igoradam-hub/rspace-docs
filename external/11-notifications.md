...
![Страница «Профиль» в кабинете: блок привязки Telegram-бота, реф.ссылка, промокод](../assets/screenshots/lk/lk-profile.png)
...
О: Проверьте, не заблокировали ли бота (открыть `@rspace_bot` → если там «Bot blocked» — разблокировать). Если не помогает — в профиле «Отвязать» → привязать заново.
