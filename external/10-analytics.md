...
![Дашборд кабинета: Кошелёк с балансом и балансом вывода, активный промокод, уровень подписки](../assets/screenshots/lk/lk-dashboard.png)

![Блок «Кошелёк» крупно: 99 516 баллов, 89 000 рублей, статистика входящих денег](../assets/screenshots/lk/lk-dashboard-wallet.png)
...
- ДомКлик — не трекается в RSpace (смотрите в их кабинете).
...
Из CusDev известно, что риелторы хотят:
- **Скоринг лида** — «горячий» или «холодный» (Михаил смотрел объект 3 раза → горячий).
- **Воронка сделок** — от лида до заключения (пока — в AmoCRM).
...
- **В AmoCRM** — если подключили интеграцию, все лиды приезжают туда автоматически.
...
- **Аналитика ДомКлик** — не трекаем в RSpace.
- **Скоринг лида** — планируется.
- **Дашборд «ваш месяц»** с графиками — в разработке (см. паспорт v1.0).
- **Экспорт в Excel** — через поддержку, не в UI.
- **Frankenstein-таблица**: ранее использовалась сводная таблица метрик, сейчас заменяется на rspace-analytics (внутренний dashboard для команды RSpace, не для агентов).
