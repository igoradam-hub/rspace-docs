...
![Список объектов в кабинете: каждая карточка показывает площадь, этаж, адрес, цену и статистику (просмотры, контакты, избранное); статус «В рекламе» означает что объект активен на классифайдах](../assets/screenshots/lk/lk-objects-list-2026-04-30.jpg)

В кабинете отображается **сводный статус** — комбинация состояния самого объекта и статусов на каждой площадке (Авито, ЦИАН):

| Сводный статус | Что означает |
|---|---|
| **Черновик** | Заполняете, не отправлен на площадки |
...
> Технически в системе два уровня статусов: «статус самого объекта» (Черновик / Создан / Архив) и «статус публикации на каждой площадке» (На модерации / Опубликован / Отклонён). Кабинет показывает сводный, чтобы не путать пользователя.
