...
> **Для кого:** риелтор, работающий с объектами; сотрудник поддержки, отвечающий на вопросы по карточке объекта.
...
2. **Параметры объекта.** Конкретные поля зависят от типа — для квартиры, например: количество комнат, общая/жилая/кухонная площади, этаж, этажей в доме, тип санузлов, вид из окна, тип ремонта, балкон/лоджия.
...
![Список объектов риелтора: карточка со статусом «Готов к рекламе»](../assets/screenshots/lk/lk-objects-list.png)

## Редактирование опубликованного

![Форма редактирования объекта: accordion из 6 секций, кнопка «Опубликовать», справа — блок «Ускорьте продажу»](../assets/screenshots/lk/lk-object-edit.png)

Изменения в опубликованном объекте синхронизируются с площадками:
- **Авито, ЦИАН** — обновление через API, обычно в течение часа.
- **ДомКлик, Яндекс.Недвижимость** — пока не подключены к ЛК, появятся в ближайшем обновлении.
...
- [Публикация на порталах](./04-publishing.md) — детали Авито/ЦИАН (+ план по ДомКлик и Яндекс.Недвижимости), платные услуги площадок.
- [Лиды](./05-leads.md) — что делать с входящими (Волна 3).
...
- **3D-планировки через AI** — есть как отдельная услуга (500 ₽ со скидкой по тарифу), не интегрированы в карточку (заказ через раздел «Услуги»).
