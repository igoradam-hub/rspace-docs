...
> **Направление:** outbound (feed, pull со стороны ДомКлик)
> **Статус:** production (через feed; direct API — нет)

## Назначение

Публикация объектов на ДомКлик — маркетплейс недвижимости от Сбербанка. Особенность: у ДомКлик **нет публичного API для создания объявлений** — площадка использует модель «feed»: наш backend отдаёт XML-файл со всеми активными объектами, ДомКлик раз в N часов забирает и обновляет свою базу.
