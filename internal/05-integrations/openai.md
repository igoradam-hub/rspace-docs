...
Две задачи:
1. **Генерация описаний объектов** для публикации (см. [realty-prompts.md](../02-modules/realty-prompts.md)) — на всех тарифах.
2. **AI-юрист «Грут»** — чат-помощник по юридическим вопросам (только Премиум+).

Обе задачи работают через один клиент — `OpenAiService`.
...
- **Модели в использовании (подтверждено в админке 2026-04-23):**
  ...
  - **AI-юрист Грут:** отдельного промпта в админке не видно — вероятно, использует ту же модель или настраивается в коде через `app/OpenAi/`.
...
### 2. AI-юрист (предположительно)

Flow не найден напрямую в коде (текущий фокус — описания), но архитектурно:

```
User → POST /services/ai-lawyer/chat {question, context?}
Backend → гейтинг: проверка Premium/Ultima
       → OpenAiService::generate(модель=GPT-4o, system-prompt юрист-эксперт)
       → ответ в чат
       → история сохраняется (TBD: где?)
```

**Статус TBD** — endpoint AI-юриста в `routes/api.php` не видно явно; возможно это часть `/services/*` или реализуется через другой механизм. Проверить.
...
- **AI-юрист endpoint** — архитектура не видна в коде, требует уточнения. Возможно реализован, но ещё не задокументирован, или в разработке.
- **История чатов AI-юриста** — где хранится, TBD.
