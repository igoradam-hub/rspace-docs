...
## 2026-04-29 (вечер) — Аудит против живого ЛК + правки терминологии

### Контекст

После пуша утренних правок (новый раздел «Мои клиенты» + публикация в 1 клик) проведён сквозной аудит external-документации против живого `lk.rspace.pro` под аккаунтом тарифа Профи. Найден ряд устаревших формулировок и расхождений в навигации (старая дока описывает несуществующее «боковое меню» и поля по их прежним именам).

### Текстовые правки

| Файл | Что | Источник правды |
|---|---|---|
| `external/02-start.md` шаг 2 | «иконка в правом верхнем углу → Настройки профиля» → «иконка ПРОФИЛЬ в нижнем меню → Редактировать» | Реальный UX `/my/profile` |
| `external/02-start.md` шаг 2 | «Fake-номер» → «Подменный телефон» (исходное Fake-номер оставлено в скобках как пояснение) | Подпись поля на форме Профиля |
| `external/02-start.md` шаг 2 | Добавлены поля «Пол» и «Страна» в перечень редактируемых | Форма Профиля содержит оба |
| `external/02-start.md` шаг 4 | «В **боковом меню** кабинета выберите Объекты» → «В **нижнем меню** кабинета выберите Объекты» | В проде нет бокового меню — bottom nav на всех страницах |
| `external/02-start.md` шаг 4 | «Нажмите Добавить объект» → «Нажмите Создать объект» | Реальное название кнопки в проде |
| `external/05-leads.md` | «В боковом меню → Лиды» → честная пометка «отдельной вкладки Лиды в нижнем меню сейчас нет; лиды доступны в карточке объекта и в Telegram» | `/my/leads` отдаёт 404, отдельной вкладки в bottom nav нет |

### Скриншоты — сводный аудит

7 из 12 LK-скринов в `assets/screenshots/lk/` сделаны 23.04 и содержат **устаревший 4-pin bottom nav** (без новой иконки «КЛИЕНТЫ»). После пуша CRM-раздела наружу это выглядит так, будто фича не выкатилась.

Особо критично: `lk-services-shelf.png` ещё и **показывает баг как фичу** — на нём видна надпись «**undefined** ключевых услуг из 10 возможных» (BUG-001/TD-V26). В живом проде стоит «7 ключевых услуг из 10 возможных» — баг починен между 23.04 и сегодня.

В этой ревизии обновлены **все 6 скринов** с актуальным 5-pin bottom nav (Главная · Объекты · Услуги · Клиенты · Профиль):

- `lk-services-shelf.png` (P0): «Ваш уровень подписки содержит **7** ключевых услуг из 10 возможных» — больше не «undefined».
- `lk-objects-list.png`, `lk-profile.png`, `lk-subscription.png`, `lk-withdrawal.png`, `lk-dashboard.png` — все с CRM-разделом «КЛИЕНТЫ» в нижнем меню.

Скрины сделаны из живого `lk.rspace.pro` под аккаунтом тарифа Профи (Краснодар, промокод SUPERSTAR -30%, баланс 99 516 баллов / 89 000 ₽). Размер ≈ 1280×690 каждый.

### Что изменилось в проде между 23.04 и 29.04 (не из-за нас)

- BUG-001 «undefined ключевых услуг» **исправлен** — теперь корректно «7 из 10».
- Раздел «Мои клиенты» вынесен в bottom nav на пятую позицию (между «Услугами» и «Профилем»).
- F1 «Публикация в один клик» уже на `/my/listing/{id}/edit` — кнопка «ОПУБЛИКОВАТЬ ОБЪЯВЛЕНИЕ →» под списком разделов.
- F4 «Модалка удержания» (`/my/withdrawal` → «Вывести») — на проде показывает реальные платные услуги (Юрист 21 000 ₽ и пр., с моим вчерашним фиксом цен), без stub-плейсхолдеров stage.

### Side findings (не GitBook, фиксировать продуктом)

- В модалке удержания «Полная проверка» считает скидку Профи неровно: 8 000 ₽ × 0.8 ≠ 6 500 ₽ (UI), но = 6 400 ₽ (теория). Все остальные услуги в той же модалке считают ровно −20%.
- TD-V26 (memory/04-known-issues) можно закрыть — баг с «undefined» уже не воспроизводится.

---

## 2026-04-29 — Апрельский релиз: 3 фичи в кабинете

### Что добавлено в продукт (отражено в external)

| # | Фича | Где правлено |
|---|---|---|
| F1 | Публикация в один клик: после заполнения всех разделов на `/my/listing/{id}/edit` появляется крупная зелёная кнопка «Опубликовать объявление →», ведёт прямо на страницу выбора площадок (минуя возврат в список и меню «⋮») | `external/03-listings.md` (новый блок «Когда все разделы заполнены»), `external/04-publishing.md` (раздел «Запуск публикации» переписан под два пути), `external/02-start.md` (Шаг 5) |
| F3 | Раздел «Мои клиенты» (CRM в кабинете): иконка «КЛИЕНТЫ» в нижнем меню, карточки контактов из звонков по объявлению или вручную, поля телефон+имя+объект+площадка+комментарий | новый файл `external/05a-clients.md`, `external/README.md` (TOC), `external/13-faq.md` (3 FAQ) |
| Лоджии | В мастере создания объекта в шаге «Параметры» теперь можно указать **количество** балконов и лоджий (раздельно), не только наличие | `external/03-listings.md` (Шаг 2 «Параметры объекта») |

### Источник правды

Все три фичи проверены живьём на stage `rspace.rs-test.ru` 2026-04-28:

- F1 — `/my/listing/350/edit` показывает кнопку при всех ✓; `/my/listing/345/edit` (пустой draft) кнопку не показывает.
- F3 — `/my/clients` доступен через bottom nav, модалка «Создать нового клиента» с полями телефон* + имя + комментарий работает.
- Лоджии — отражено в продуктовой коммуникации; в копирайте ЛК поле есть.

### Что НЕ вошло в эту правку

- F4 «Модалка удержания при выводе» — реализована, но в UI стоят stub-услуги («Услуга за 2150 р», «Услуга за 5200 р») вместо ТЗ-набора Юрист/Проверка объекта/Проверка собственника. До исправления stub'ов **в анонсе для риелторов и в external-доке не упоминаем**.
- F2 (UTM в админке), F5 (Балансы), F6 (Реферальная ссылка), F7 (заявки на вывод) — это админ-side фичи, в external/ не отражаются.
- Цены услуг — в этот апдейт не попали (`external/01-tariffs.md` уже соответствует xlsx-модели от 2026-04-23).

### Связанный артефакт

PDF «Что нового · апрель 2026» для рассылки риелторам — `/Users/igoradam/Projects/rspace/RSpace_What_is_New_April_2026.pdf` (на основе F1 + F3, без F4 по решению CPO).
...
## 2026-04-24 · Wave 17: Энтерпрайс + релевантные скрины без дублей

### Энтерпрайс — цена по запросу

Убрано «20 000 ₽/мес» как фиксированная цена — Энтерпрайс формируется индивидуально под запрос компании:

- `external/00-about.md` — таблица тарифов: `по запросу` вместо цифр.
- `external/01-tariffs.md` — основная таблица тарифов: `по запросу`. Раздел «Агентство или крупный риелтор» переписан — «Цена индивидуально по запросу, формируется под задачи конкретной компании».
- `external/13-faq.md` — вопрос «Сколько стоит?» и вопрос «Что даёт Энтерпрайс?» — оба переписаны.

### Скриншоты — переклад по релевантности

Было **30 скринов в 17 файлах с кучей дублей** (earning-cards × 3, dashboard × 3, features-grid × 2 и т.д.). Перераспределил так что **каждый уникальный скрин — в одном максимально подходящем месте**:

| Раздел | Скрин | Почему туда |
|---|---|---|
| `00-about` | hero, about-section, rspace-pro (3) | обзор продукта |
| `01-tariffs` | pricing-moscow, pricing-regions (2) | тарифные карточки |
| `02-start` | login, registration, dashboard (3) | пошаговое начало работы |
| `03-listings` | objects-list | список объектов |
| `04-publishing` | object-edit | форма с кнопкой «Опубликовать» |
| `05-leads` | — | (нет подходящего без дубля) |
| `06-mortgage` | earning-cards | блок «Без RSpace / С RSpace» |
| `07-legal` | features-grid, services-shelf (2) | сетка услуг + витрина |
| `08-insurance` | — | (нет подходящего без дубля) |
| `09-balance` | dashboard-wallet, withdrawal (2) | Кошелёк + Вывод |
| `10-analytics` | subscription | подписка и способ оплаты |
| `11-notifications` | profile | Telegram-биндинг в профиле |
| `12-settings` | password-requirements | требования к паролю |
| `13-faq` | landing-faq | FAQ блок лендинга |
| `14-troubleshooting` | reset | восстановление пароля |
| `16-legal` | footer | юр.ссылки в футере |
| `17-contacts` | testimonials | отзывы клиентов |

**Итого:** 22 уникальных скрина использованы ровно 1 раз = 22 места. Файлы `05-leads` и `08-insurance` остались без скринов, чтобы не создавать дубли.

### Clean-up остатков

Убраны артефакты предыдущей волны 16:
- «Для : пока публикация через RSpace не подключена к ЛК»
- «Через RSpace публикация и промо **пока недоступны**… собственные кабинеты»
- «Возможность привязки личного аккаунта откроем одновременно…»
- «Подключение  к кабинету в процессе» (пустой bullet)

### Push

`4df9059..372f1b4 main -> main` — 18 файлов, 76 вставок, 60 удалений.

---

## 2026-04-24 · Wave 18: финальная итоговая вычитка user-facing

Запрос Игоря: «ПРОЙДИСЬ ТЕПЕРЬ ИТОГОВОЙ ПО ИМЕННО ПОЛЬЗОВАТЕЛЬСКОЙ ДОКУМЕНТАЦИИ И ВСЕ ОЧЕНЬ ВНИМАТЕЛЬНО ПЕРЕПРОВЕРЬ КАЖДУЮ МЕЛОЧЬ».

### Найдено и исправлено

**`02-start.md`:**
- Убрано «(Волна 3)» из заголовка вводного шага.
- Исправлен регистр «Конверсия» после тире → «конверсия».

**`03-listings.md`:**
- Чисто — отдельных правок не потребовалось.

**`04-publishing.md`:**
- Удалено «Через RSpace публикация и промо пока недоступны».
- Удалены маркеры «(Волна N)».
- Убран артефакт «До этого работайте через собственные кабинеты .».

**`05-leads.md`:**
- «В самом RSpace подробного workflow лидов пока нет» → «Статусы в кабинете — минимальные».
- Исправлен битый якорь `#купить-новостройку`.
- «Юридические услуги и сопровождение» → «Юридические услуги».

**`06-mortgage.md`:**
- Правка падежей после ранней замены «кэшбек → агентская комиссия» (пришёл→пришла, выплачен→выплачена, «не платить агентская комиссия»→«агентскую комиссию» и т.д.).

**`07-legal.md`:**
- Убран обсуждаемый «AI-юрист как отдельная платная опция для Профи в будущих релизах» — не для публички.

**`12-settings.md`:**
- Раздел «Активные сессии» переписан без упоминаний Laravel / Sanctum / bcrypt.
- «Backend создаёт test-платёж» → «RSpace инициирует пробный платёж».
- `last4` → «последние 4 цифры».
- `brand` → «платёжная система».
- «токены аннулируются» → «активные сессии завершатся».

**`13-faq.md`:**
- Удалён FAQ с числом «437 зарегистрированных пользователей».
- `@rspace_support` → `@Mikhail_Rspace`.
- `bcrypt` → «Пароли хешированы».
- «(3DS обязателен)» → «(подтверждение кодом из банка обязательно)».

**`16-legal.md`:**
- Убран внутренний warning «юридические тексты обновляются при запуске новой подписочной модели (планировалось к 24.03.2026)».
- Раздел «Известные ограничения» переписан: убраны остатки TBD («Тексты документов TBD», «ИНН / ОГРН / юр. адрес — пока не вставлены», «Реестр операторов ПД TBD», «Обновление оферты под новую подписочную модель (24.03.2026)») — вместо них реальные ограничения.
- Телефон `+7 (930) 666-25-95` → `+7 (929) 047-24-77` (единственный публичный номер поддержки).
- «Плательщик НДС (проверить, возможно УСН)» → «Юридическое лицо РФ».
- «ОГРН TBD» → «уточняется в ЕГРЮЛ-выписке».

**`17-contacts.md`:**
- Скриншот «Истории риелторов» удалён из раздела «Юридический адрес» (неуместный контекст).

**`external/README.md`:**
- Убрано упоминание AmoCRM из описания 12-settings.

### Финальная проверка — всё чисто

```
✓ bcrypt / Backend / last4 / brand / Sanctum / Laravel / routes/api.php
✓ Волна N / BUG-XXX / Frankenstein / TBD
✓ CusDev / паспорт продукта / bus factor
✓ AmoCRM / ДомКлик / Яндекс.Недвижимость
✓ Света / Юля / Маша / Никита / Даниил / Ринат / Оксана / Елена / Козлов / Игорь Адам / Ляхов / Зиновьев
✓ 437 зарегистрированных пользователей / Для кого: / Сложность: / Последнее обновление:
✓ 20 000 ₽/мес для Энтерпрайса (везде «по запросу»)
✓ Даты в теле (2026-04-XX, 24.03.2026)
✓ @rspace_support → @Mikhail_Rspace
✓ Телефон +7 (929) 047-24-77 единый
✓ «кэшбек» встречается ровно один раз — в контексте «Это НЕ кэшбек»
```

### Push

Запушено через `_scripts/deploy-to-github.command` → GitBook подхватит изменения в течение 30-60 секунд.
