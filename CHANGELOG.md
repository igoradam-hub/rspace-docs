...
В этой ревизии обновлён только `lk-dashboard.png` (свежий 5-pin nav). Остальные 5 (`lk-services-shelf`, `lk-objects-list`, `lk-profile`, `lk-subscription`, `lk-withdrawal`) переснимать стандартными js-инструментами (`html2canvas`, `html-to-image`) не удалось — обе библиотеки спотыкаются о современный CSS `color()` в production-стилях RSpace и/или зависают на промисе. Откладываем на отдельную сессию через DevTools `Page.captureScreenshot` (CDP) или Puppeteer на хосте.
